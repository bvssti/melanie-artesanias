...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-bg-card shadow-[var(--shadow-lg)] flex flex-col data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-300",
  {
    variants: {
      side: {
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute right-4 top-4 rounded-full p-2 hover:bg-muted transition-colors focus:outline-3 focus:outline-accent focus:outline-offset-2">
        <X className="h-5 w-5" />
...
export const SheetFooter = ({
  ...
  <div
    className={cn(
      "p-6 border-t border-border flex flex-col gap-3",
      className
    )}
    {...props}
  />
